...
# astropy (5.0.3), matplotlib (3.9.2), numpy (1.26.4), opencv-python (4.10.0.84), rawpy (0.23.1), scipy (1.13.1)

import copy, cv2, glob, sys, os, re, numpy as np, rawpy, warnings, time
...
def show_image(image, gamma=0, stars=[], colors=[]):
    ...
    plt.get_current_fig_manager().window.wm_iconbitmap(GUINAME + ".ico")
    if gamma > 0:
...
class NewGUI():
    def __init__(self):
        ...
        self.button_load.grid(row=0, column=0, sticky='NWSE', padx=padx, pady=pady)
        self.button_show = tk.Button(text="Show", command=self.process_show)
        self.button_show.grid(row=0, column=1, sticky='NWSE', padx=padx, pady=pady)
        self.label_files_var = tk.StringVar()
        self.label_files = tk.Label(textvariable=self.label_files_var, justify='left')
        self.label_files.grid(row=1, column=0, columnspan=2, sticky='NWSE', padx=padx, pady=pady)
        self.update_labels()
        self.button_start = tk.Button(text="Start", command=self.process_start)
        # self.button_start = tk.Button(text="Start", command=lambda: threading.Thread(target=self.process_start).start())
        self.button_start.grid(row=2, column=0, sticky='NWSE', padx=padx, pady=pady)
        self.button_stop = tk.Button(text="Stop", command=self.stop)
        self.button_stop.grid(row=2, column=1, sticky='NWSE', padx=padx, pady=pady)
    ...
    def load_files(self):
        if self.running:
            tk.messagebox.showerror('Error', 'Program is running. Wait for it to finish or stop it!')
            return
        asked_files = askopenfilename(initialdir=self.lastpath, multiple=True,
              filetypes=[('all', '.*'), ('.arw', '.arw')])
        if asked_files:
            self.loaded_files = asked_files
    ...
    def set_parameters(self):
        # determine region
        self.region_name = self.radio_region.get()
        if contains(self.region_name, "edge"):
            self.region = -1
        elif contains(self.region_name, "center"):
            self.region = 0
        elif contains(self.region_name, "(TL)"):
            self.region = 1
        elif contains(self.region_name, "(TR)"):
            self.region = 2
        elif contains(self.region_name, "(BL)"):
            self.region = 3
        elif contains(self.region_name, "(BR)"):
            self.region = 4
        else:
            self.region = 0
    ...
    def process_start(self):
        ...
        try:
            ...
            # write header
            savepath = os.path.dirname(self.loaded_files[0])
            with open(savepath + os.sep + GUINAME + ".csv", 'w') as f:
                f.write("#timestamp: " + datetime.now().strftime("%Y-%m-%d %H:%M:%S") + "\n")
                f.write("#region_name: " + self.region_name + "\n")
                f.write("#region_size: " + str(self.region_size) + "\n")
                f.write("#analyze_colors: " + str(self.analyze_colors) + "\n")
                header = ["FILE", "IMAGE"]
                if len(self.analyze_colors) > 1:
                    for i in range(len(self.analyze_colors)):
            ...
            for i in range(len(self.loaded_files)):
                ...
                self.update_labels(status=
                    os.path.basename(file) +
                    " (" + str(i+1) + "/" + str(len(self.loaded_files)) + ")"
                )
                res = process_file(
                    self.loaded_files[i],
                ...
                with open(savepath + os.sep + GUINAME + ".csv", 'a') as f:
                    file_abab_array = abab_array([files_stars[-1], files_widths[-1], files_aspects[-1]])
                    f.write(csv_sep_line([files_nr[-1]] + [files_imagenr[-1]] + file_abab_array))
